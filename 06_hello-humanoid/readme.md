...
### Download
Once we have applied an (highlight:orange text:`animation`) to our (highlight:red text:`character`) we are now ready to (highlight:yellow text:`download`) both and import them as a single entity into Unity. In the download settings window, you will need to select (highlight:fuchsia text:`FBX for Unity`) as your format. You can leave the other settings alone (`with skin`, `30 frames per second`, `no keyframe reduction`), and then (highlight:brown text:`download`) the file.
